...
def plot_cx(rootfile):
   ...
   #tag1.Draw()
   #tag2.Draw()
   tag3 = ROOT.TLatex(lab_x0,lab_y0-0.03,"m_{A'} = 100 MeV")
   tag3.SetNDC()
   tag3.SetTextFont(52)
   tag3.SetTextSize(0.05)
   tag3.Draw()
   tag4 = ROOT.TLatex(lab_x0-0.005,lab_y0-0.09,"Tungsten target")
   tag4.SetNDC()
   tag4.SetTextFont(52)
   ...
   xlabel.AddText("e^{-} Incident Energy (GeV)")
   xlabel.SetFillColor(0)
   xlabel.SetBorderSize(0)
   xlabel.Draw()
   img = ROOT.TImage.Create()
   img.FromPad(plot)
   img.WriteImage("cx_comp_0p1_W.png")
   plot.Print("cx_comp_0p1_W.pdf")
   plot.Print("cx_comp_0p1_W.C")
