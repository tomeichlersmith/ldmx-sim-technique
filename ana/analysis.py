...
def get_dark_brem_products(event) :
    ...
    recoil = None
    aprime = None
    for id_p in event.SimParticles :
        if id_p.second.getProcessType() == dark_brem_process_type :
            if id_p.second.getPdgID() == 622 :
                if aprime is not None :
                    raise Exception(f'More than one A\' have been found in event {event.EventHeader.getEventNumber()}.')
                aprime = id_p.second
    ...
    if aprime is None :
        raise Exception(f'No A\' found in {event.EventHeader.getEventNumber()}')
    elif recoil is None :
        raise Exception(f'No recoil electron found in {event.EventHeader.getEventNumber()}')
